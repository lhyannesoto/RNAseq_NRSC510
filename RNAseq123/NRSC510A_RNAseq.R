...
#create a variable making the gene names the first column and the ID as the second column
#I think we are trying to save the genenames because putting them away later
...
##getting rid of low expressed genes (cutoff for the number of samples that we want above 3, getting rid of things that are less than one group)
cutoff <- 3
drop <- which(apply(cpm(d0), 1, max) < cutoff) #drop prints out the row number to drop
#cpm looks at normalized
#try using the filter function and try to get the same thing
#this is getting rid of the IDs?
d <- d0[-drop,]
dim(d) #dim shows you dimensions of the object, so this is number of genes left

#filtering plot
library(RColorBrewer)
#ncol returns the number of rows or columns present in d0
#so nsamples is now the number of columns in d0
nsamples <- ncol(d0)
